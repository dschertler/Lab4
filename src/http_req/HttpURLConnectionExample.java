...
public class HttpURLConnectionExample {
	...
	private void sendGet() throws Exception {
                //getgrade?xml=<tuid>4564</tuid>getgrade?xml=<tuid>4564</tuid>
		//String url = "http://129.32.92.49:8080/xml_lab/time";
                System.out.println("Enter TUID");
                Scanner scan = new Scanner(System.in);
                int input = scan.nextInt();
                String url = "http://129.32.92.49:8080/xml_lab/getgrade?xml=<tuid>"+input+"</tuid>";
		URL obj = new URL(url);
		...
		System.out.println(response.toString());
                   String inp = response.toString();
                   DocumentBuilderFactory dbFactory = DocumentBuilderFactory.newInstance();
                   DocumentBuilder dBuilder = dbFactory.newDocumentBuilder();
                   Document doc = dBuilder.parse(new InputSource(new StringReader(inp)));
                   doc.getDocumentElement().normalize();
                   String grade = doc.getElementsByTagName("content").item(0).getTextContent();
                   System.out.println("Your grade is:" +grade);
                
              /*  for(int i = 0; i < response.toString().length(); i++){
                    char k = response.toString().charAt(i);
                    if(k == 'A' || k == 'B' || k == 'C'|| k == 'D' || k == 'F'){
	...
	private void sendPost() throws Exception {
		...
		System.out.println(response.toString());
                ...
                // String timeStamp = doc.getElementsByTagName("content").item(0).getTextContent();
                   String grade = doc.getElementsByTagName("content").item(0).getTextContent();
               //    System.out.println("timestemp " + timeStamp);
